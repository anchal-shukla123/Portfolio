...
const EducationCard = ({ item, index }) => (
  <div
    ...
    className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:border-red-500/30 hover:shadow-[0_15px_35px_rgba(255,42,42,0.1)] transition-all duration-500"
  >
    <div className="flex items-start justify-between gap-4 mb-6">
      <span className="text-5xl font-black text-white/10 font-serif italic">0{index + 1}</span>
      <span className="text-red-400 text-xs font-bold font-mono tracking-wider uppercase text-right">{item.duration}</span>
    </div>
    <h3 className="text-white text-xl md:text-2xl font-black mb-2 tracking-tight">
      {item.degree}
    </h3>
    <p className="text-white/60 text-sm md:text-base font-medium mb-5">
      {item.institution}
    </p>
    <span className="inline-block bg-[#ff2a2a]/20 text-[#ff2a2a] text-xs font-black tracking-widest uppercase py-2 px-4 rounded-full border border-[#ff2a2a]/30">
      {item.score}
    </span>
  </div>
);
